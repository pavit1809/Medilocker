...
export const addUserToAcl = async (doctorId, documentId, currentUser) => {
  const acl = await Acl.findOne({documentId: documentId});
  const doctor = await Doctor.findById(doctorId);
  sendEmail(doctor.email, `Document Shared by ${currentUser.name}`, `A Document is shared with your medical account on Medilocker by ${currentUser}`);
